...
{
    public class NavigationService<T> : INotifyPropertyChanged
    {
        /// <summary>
        ...
        public void NavigateTo(string pageKey, object parameter)
        {
            if (pageKey != CurrentPageKey)
            {
                lock (pagesByKey)
                {
                    ...
                    var label = GetDescendantFromName(Application.Current.MainWindow, "PageTitle") as Label;

                    if (frame != null)
                    {
                        frame.Source = pagesByKey[pageKey];
                    }
                    if (label != null && parameter != null)
                    {
                        var title = parameter as PageItem;
                        label.Content = title.PageTitle;
                    }
                    Parameter = parameter;
